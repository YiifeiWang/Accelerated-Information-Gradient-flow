function [Xs,out] = AIG_SE(X_init, dlog_p, opts)
%%%---------------------------------------------%%%
% This implements the the particle version of AIG flow
% 
% Input:
...
%%%---------------------------------------------%%%
	...
	for iter = 1:iter_num
		...
		ibw = ibw_out;
		if opts.strong
			V = coef*V+sqtau*grad-sqtau*xi;
		else
			V = (k-1)/(k-1+opts.r)*V+sqtau*grad-sqtau*xi;
			k = k+1;
		end
		if trace(V'*(grad-xi))<0 && opts.restart
			V = sqtau*(grad-xi);
			k = 1;
		end
		X = X+sqtau*V;
